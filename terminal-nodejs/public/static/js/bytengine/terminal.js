...
$(function(){
    ...
    var TerminalView = Backbone.View.extend({
        ...
        loginRequest: function(username, password, process){
            ...
            $.ajax({
                type:'POST',
                url: 'http://' + location.host + '/bytengine/login',
                data: postdata,
                dataType:'json',
                context: this,
        ...
        helpCommandRequest: function(command, process){
            $.ajax({
                type:'GET',
                url: 'http://' + location.host + '/bytengine/docs/'+command,
                dataType:'html',
                success: function(data)
                {
                    process(data);                    
        ...
        commandRequest: function(script, context, retrycount, process){
            if (retrycount < 0) {
                ...
                $.ajax({
                    type:'POST',
                    url: 'http://' + location.host + '/bytengine/run',
                    data: postdata,
                    dataType:'json',
                    context: context,
                    success: function(data)
                ...
                source: function(query,process) {
                    ...
                    try {
                        ...
                        if (isdb_command && pattern != null) {
                            _.each(COMMAND_LIST.user,function(item){
                            ...
                            _.each(COMMAND_LIST.admin,function(item){
                                var result = item.match(pattern);
                                if (result != null) {
                                    data.push(item);
                                }
    ...
    $.ajax({
        type:'GET',
        url: 'http://' + location.host + '/bytengine/docs/',
        dataType:'json',
        success: function(data)
        {
            COMMAND_LIST = data;            
